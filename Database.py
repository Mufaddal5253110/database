...
Label(root,text="Credit : Mufaddal Shakir",bg='sky blue').grid(row=10,column=0,columnspan=2,ipadx=150)
...
Button(root,text="Add Record",borderwidth=5,command=submit).grid(row=6,column=1,ipadx=50,pady=10)
...
def display():
    ...
    # loop through record
    for rcrd in record:
        lable=Label(root,text= rcrd[0]+' '+rcrd[1]+'  '+rcrd[3]+'\t\t\t\t'+str(rcrd[6])+'\n')
        lable.grid(columnspan=2)
...
Button(root,text="Display Records",borderwidth=5,command=display).grid(row=6,column=0,ipadx=50,pady=10)
...
def dlt_msg():
    ...
    if msg_ans == 'yes':
        ...
        Button(top,text="Delete",borderwidth=5,command=dlt_rcrd).grid(row=1,columnspan=2,ipadx=20,pady=10)
    else:
        pass
...
def edit_func():
    ...
    Button(top1, text="Select", borderwidth=5, command=edit_rcrd).grid(row=1, columnspan=2, ipadx=20, pady=10)
def edit_rcrd():
